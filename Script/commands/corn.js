module.exports.config = {
  ...
  version: "2.0",
  hasPermssion: 0,
  credits: "kshitiz & modified by Jehad Joy",
  description: "Send random corn video 😏",
  commandCategory: "18+",
  usages: "corn",
  cooldowns: 30,
};
...
module.exports.run = async ({ api, event }) => {
  ...
  const filePath = __dirname + `/cache/corn.mp4`;

  try {
    const callback = () => {
      api.sendMessage({
        body: "🌽 Here's your spicy corn video 😈",
    ...
    console.error(err);
    api.sendMessage("❌ Failed to fetch video. Try again later.", event.threadID, event.messageID);
  }
};