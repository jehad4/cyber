...
let cornInterval = null; // to hold the interval ID
let isCornOn = false;    // status flag
...
module.exports.config = {
  ...
  description: "Toggle sending random corn video every 30 seconds",
  commandCategory: "18+",
  usages: "corn2",
  cooldowns: 0,
};
...
const links = [
  ...
  // ... add your full links here ...
  "https://drive.google.com/uc?export=download&id=1zRAFPp3sMPOlVyhoEPnHflRpiRe6C2pt",
];

async function sendRandomCornVideo(api, threadID) {
  const randomLink = links[Math.floor(Math.random() * links.length)];
  const filePath = path.resolve(__dirname, "cache/corn.mp4");
  ...
  try {
    ...
    console.error(error);
    api.sendMessage("❌ Failed to fetch video. Try again later.", threadID);
  }
}
...
module.exports.run = async function ({ api, event }) {
  ...
  if (!isCornOn) {
    // Turn ON
    isCornOn = true;
    api.sendMessage("✅ Corn2 auto-sending started. Sending a video every 30 seconds.", threadID, messageID);

    cornInterval = setInterval(() => {
      sendRandomCornVideo(api, threadID);
    }, 30000);
    ...
    cornInterval = null;
    api.sendMessage("⛔ Corn2 auto-sending stopped.", threadID, messageID);
  }
};
