...
var min = Math.floor(Math.random() * 2);
  var data = await Currencies.getData(event.senderID);
   ...
   var callback = () => api.sendMessage({body:`Ảnh NUDE !!!!\nSố Ảnh: ${link.length}\n-1000 đô !`,attachment: fs.createReadStream(__dirname + "/cache/3553.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/3553.jpg"), event.messageID); 
      return request(encodeURI(link[Math.floor(Math.random() * link.length)] + (max - min))).pipe(fs.createWriteStream(__dirname+"/cache/3553.jpg")).on("close",() => callback());
     }
   };